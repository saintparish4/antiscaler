...
function defaultCommand(
	...
	if (task === "build" && framework === "next")
		return `${frameworkBin(pm)} next build`;
	if (task === "build" && framework === "vite")
		return `${frameworkBin(pm)} vite build`;
...
export async function registerInitAction(): Promise<void> {
	...
	if (!process.stdin.isTTY) {
		...
		const tasks = [
			{
				name: "build",
				command: defaultCommand(pmName, "build", frameworkName),
				inputs: DEFAULT_INPUTS["build"] ?? [],
			},
		];
		writeFileSync(path.join(cwd, "antiscale.config.ts"), buildTemplate(tasks));
		console.log(
			`Created antiscale.config.ts (non-interactive, detected ${pmName}${frameworkName ? `/${frameworkName}` : ""})`,
		);
		return;
	}
	...
	try {
		...
		const rawTasks = await rl.question(
			`Tasks to configure [${defaultTaskList}]: `,
		);
		...
		for (const name of taskNames) {
			...
			const rawCmd = await rl.question(
				`  Command for "${name}" [${suggested}]: `,
			);
			const command = rawCmd.trim() || suggested;
			const suggestedInputs = (
				DEFAULT_INPUTS[name] ??
				DEFAULT_INPUTS["build"] ??
				[]
			).join(", ");
			const rawInputs = await rl.question(
				`  Inputs for "${name}" [${suggestedInputs}]: `,
			);
