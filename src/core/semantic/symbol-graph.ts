 ...
 * notes. The index lives at `.antiscale/graph/symbols.json` and is updated
...
export interface BuildSymbolGraphOptions {
	...
	include?: string[];
	/** Extra ignore globs on top of node_modules/.git/.antiscale/dist. */
	ignore?: string[];
...
export function defaultGraphDir(cwd: string): string {
	return path.join(cwd, ".antiscale", "graph");
}
 ...
 * and save the result back to `graphDir` (default `.antiscale/graph/`).
 */
